...
ui <- page_sidebar(
  ...
  sidebar = sidebar(width = "380px",
    ...
    tabsetPanel(
      tabPanel("Analysis", useShinyjs(), br(),
               ...
               conditionalPanel(
                 ...
                 fluidRow(
                   column(10, fileInput(inputId   = "files",
                                        ...
                                        multiple  = TRUE,
                                        accept    = c(".txt", ".tsv"))),
                   column(2, actionButton(inputId = "showExample", 
                                         label    = "", 
                   ...
                   fluidRow(
                     column(width = 11,
                      div(switchInput(inputId  = "useExampleData",
                                      onLabel  = "Yes",
                                      offLabel = "No",
                                      value    = FALSE),
                     ...
                     column(width = 1,
                       downloadLink(outputId = "downloadExample",
                                    label    = icon("download"), 
                                    style    = "margin-left:-28px; margin-top:10px;")))),
               
               conditionalPanel(condition    = "input.inputMethod == 'paste'",
                  div(textAreaInput(inputId  = "pastedGenes",
               ...
               div(selectInput(inputId  = "metaAnalysisMethod",
  ...
  card(
    ...
    shinycssloaders::withSpinner(dataTableOutput("resultsTable")), br(),
    ...
    shinyjs::hidden(
      div(id = "excludedSection",
          ...
          dataTableOutput("excludedTable"))), br(),
    
    shinycssloaders::withSpinner(plotlyOutput("resultsDotplot", height = "700px")),
...
server <- function(input, output, session) {
  ...
  exampleFiles <- c("./example_data/genes1.txt",
                    "./example_data/genes2.txt",
                    "./example_data/genes3.txt",
                    "./example_data/genes4.txt")
  ...
  observeEvent(input$runButton, {
    ...
    # 5) Organism nomenclature
    if (!validate_organism(genes, input$organism, input$IDtype, session)) {
      resultsVal(list(included = NULL, excluded = NULL))
      return()}
  ...
  output$downloadButtonUI <- renderUI({
    ...
    fluidRow(
      column(5, downloadButton("downloadTableCSV",  "Download CSV", style = "width:100%")),
      column(5, downloadButton("downloadTableTSV",  "Download TSV", style = "width:100%")),
      column(1, actionButton("toggleExcluded",
                             label = NULL,
                             icon = icon("eye"),
                             style = "width:100%",
                             title = "Show/hide excluded terms")),
      column(1, downloadButton("downloadExcludedTermsList",
                               label = NULL,
                               icon = icon("download"),
                               style = "width:100%",
...
###############################################################################
  ...
  validate_organism <- function(gene_list, organism, ID_type, session) {
  ...
  validate_gene_ids <- function(gene_vector, ID_type, session) {
    ...
    gene_list <- as.character(gene_vector)
    ...
    # Validación extra: excluir Ensembl si el tipo es SYMBOL
    if (ID_type == "SYMBOL") {
